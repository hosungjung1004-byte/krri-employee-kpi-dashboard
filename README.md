...
Vercel에서는 서버리스 임시 저장소(`/tmp`)로 데모 DB를 실행합니다. 화면 조회와 기능 시연은 가능하지만 직원·KPI 등록 및 수정 내용은 함수 재시작이나 재배포 후 초기화될 수 있습니다. 실제 운영 시에는 PostgreSQL 등 외부 영구 데이터베이스를 연결해야 합니다.
